...
$return .= <<<EOF
, เนื้อหาถูกซ่อนไว้ คุณจะต้องมีเครดิต {$creditsrequire} เครดิต หรือมากกว่าจึงจะสามารถมองเห็นเนื้อหาที่ถูกซ่อนไว้ คุณมีเพียง {$_G['member']['credits']}</div>
EOF;
?><?php return $return;?><?php }

function tpl_hide_credits($creditsrequire, $message) {?><?php
$return = <<<EOF
<div class="locked">เนื้อหาถูกซ่อนไว้ คุณจะต้องมีเครดิต {$creditsrequire} เครดิต หรือมากกว่าจึงจะสามารถมองเห็นเนื้อหาที่ถูกซ่อนไว้</div>
{$message}<br /><br />
...
$return = <<<EOF
<div class="grey quote"><blockquote>อ้างอิง: \\1</blockquote></div>
EOF;
?><?php return $return;?><?php }
...
global $_G;?><?php
$return = <<<EOF
<div class="showhide"><h4>เนื้อหาถูกซ่อนไว้</h4>\\1</div>
...
$return .= <<<EOF
เนื้อหาถูกซ่อนไว้ คุณจะต้อง<a href="forum.php?mod=post&amp;action=reply&amp;fid={$_G['fid']}&amp;tid={$_G['tid']}" onclick="showWindow('reply', this.href)">แสดงความคิดเห็น</a>ก่อนจึงจะสามารถมองเห็นเนื้อหาส่วนที่ถูกซ่อนนี้ได้</div>
EOF;
?><?php return $return;?><?php }
...
<span class="xg1">({$attach['dateline']} อัปโหลด)</span>
</p>
<p class="xg1">{$attach['attachsize']}
EOF;
 if($attach['readperm']) { 
$return .= <<<EOF
, จำกัดระดับ: <strong>{$attach['readperm']}</strong>
EOF;
 } 
$return .= <<<EOF
, ดาวน์โหลดแล้ว: {$attach['downloads']}
EOF;
 if(!$attach['attachimg'] && $_G['getattachcredits']) { 
$return .= <<<EOF
, ดาวน์โหลดไฟล์นี้ต้องใช้เครดิต: {$_G['getattachcredits']}
EOF;
 } 
$return .= <<<EOF
</p>
...
<em class="xg1"><br />(ดาวน์โหลดแล้ว: {$attach['downloads']}, {$attach['dateline']} อัปโหลด)
</em>

EOF;
 if(!$attach['attachimg'] && $_G['getattachcredits']) { 
$return .= <<<EOF
<p>ดาวน์โหลดไฟล์นี้ต้องใช้เครดิต: {$_G['getattachcredits']}</p>
EOF;
 } 
$return .= <<<EOF
