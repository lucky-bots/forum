...
0
|| checktplrefresh('./template/default/mobile/forum/discuz.htm', './template/default/mobile/common/header.htm', 1649492671, 'diy', './data/template/1_diy_mobile_forum_discuz.tpl.php', './template/default', 'mobile/forum/discuz')
|| checktplrefresh('./template/default/mobile/forum/discuz.htm', './template/default/mobile/common/footer.htm', 1649492671, 'diy', './data/template/1_diy_mobile_forum_discuz.tpl.php', './template/default', 'mobile/forum/discuz')
;?>
...
<title><?php if(!empty($navtitle)) { ?><?php echo $navtitle;?> - <?php } if(empty($nobbname)) { ?> <?php echo $_G['setting']['bbname'];?> - <?php } ?> อุปกรณ์พกพา - Powered by Discuz!</title>
<style type="text/css">
...
<div class="hd"><a href="<?php echo $nav;?>" title="<?php if(empty($nobbname)) { ?> <?php echo $_G['setting']['bbname'];?> - <?php } ?>อุปกรณ์พกพา"><?php if($_G['setting']['mobile']['mobilesimpletype'] == 1) { if(empty($nobbname)) { ?> <?php echo $_G['setting']['bbname'];?> - <?php } ?> อุปกรณ์พกพา<?php } else { ?><img src="./static/image/mobile/logo.gif" /><?php } ?></a></div>
<div class="wp">
<div class="pd2">
<?php if($_G['uid']) { ?><a href="home.php?mod=space&amp;uid=<?php echo $_G['uid'];?>&amp;do=profile"><?php echo $_G['member']['username'];?></a><span class="pipe">|</span><a href="home.php?mod=space&amp;do=pm" <?php if($_G['member']['newpm']) { ?>class="xi1"<?php } ?>><?php if($_G['member']['newpm']) { ?>ข้อความส่วนตัวใหม่<?php } else { ?>ข้อความส่วนตัว<?php } ?></a><span class="pipe">|</span><a href="home.php?mod=space&amp;uid=<?php echo $_G['uid'];?>&amp;do=favorite&amp;view=me&amp;type=forum">บุ๊คมาร์ก</a><span class="pipe">|</span><a href="home.php?mod=space&amp;uid=<?php echo $_G['uid'];?>&amp;do=thread&amp;view=me">โพสต์ของฉัน</a><span class="pipe">|</span><a href="member.php?mod=logging&amp;action=logout&amp;formhash=<?php echo FORMHASH;?>" title="ออกจากระบบ">ออกจากระบบ</a><?php } elseif($_G['connectguest']) { ?><?php echo $_G['member']['username'];?><span class="pipe">|</span><a href="member.php?mod=logging&amp;action=logout&amp;formhash=<?php echo FORMHASH;?>" title="ออกจากระบบ">ออกจากระบบ</a><?php } else { ?><a href="member.php?mod=logging&amp;action=login" title="ลงชื่อเข้าใช้">ลงชื่อเข้าใช้</a><?php if($_G['setting']['regstatus']) { ?><span class="pipe">|</span><a href="member.php?mod=<?php echo $_G['setting']['regname'];?>" title="<?php echo $_G['setting']['reglinkname'];?>"><?php echo $_G['setting']['reglinkname'];?></a><?php } if($_G['setting']['connect']['allow'] && !$_G['setting']['bbclosed']) { ?><span class="pipe">|</span><a href="<?php echo $_G['connect']['login_url'];?>&statfrom=login_simple">!connect_mobile_login!</a><?php } } ?>
</div>
...
<div class="bm">
<div class="bm_h">
<a href="home.php?mod=space&amp;uid=<?php echo $_G['uid'];?>&amp;do=favorite&amp;view=me&amp;type=forum">บอร์ดโปรดของฉัน</a>
</div><?php $i=1;?><?php if(is_array($forum_favlist)) foreach($forum_favlist as $key => $val) { ?><div class="bm_c <?php if($i%2 == 1) { ?>add<?php } else { ?>even<?php } ?>"><a href="forum.php?mod=forumdisplay&amp;fid=<?php echo $val['id'];?>"><?php echo $val['title'];?></a> <a href="home.php?mod=spacecp&amp;ac=favorite&amp;op=delete&amp;favid=<?php echo $val['favid'];?>&amp;type=forum" title="ยกเลิกบุ๊คมาร์ก" class="xg1">[x]</a></div>
  <?php $i++;?><?php } ?>
</div>
</div>
...
<div class="box">
วันนี้<?php echo $todayposts;?><span class="pipe">|</span>สมาชิก<?php echo $_G['cache']['userstats']['totalmembers'];?><?php if(empty($gid) && $_G['setting']['whosonlinestatus'] && $onlinenum) { ?><span class="pipe">|</span>ออนไลน์<?php echo $onlinenum;?><?php } ?>
</div>
