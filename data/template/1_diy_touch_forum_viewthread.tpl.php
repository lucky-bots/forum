...
0
|| checktplrefresh('./template/default/touch/forum/viewthread.htm', './template/default/touch/forum/forumdisplay_fastpost.htm', 1649492837, 'diy', './data/template/1_diy_touch_forum_viewthread.tpl.php', './template/default', 'touch/forum/viewthread')
|| checktplrefresh('./template/default/touch/forum/viewthread.htm', './template/default/touch/common/seccheck.htm', 1649492837, 'diy', './data/template/1_diy_touch_forum_viewthread.tpl.php', './template/default', 'touch/forum/viewthread')
;?>
...
<div class="nav">
<span class="y">
<button class="btn_pn btn_pn_blue" id="replyid"><span>ตอบกลับ</span></button>
</span>
...
<?php echo $_G['forum_thread']['subject'];?>
            <?php if($_G['forum_thread']['displayorder'] == -2) { ?> <span>(ตรวจสอบ)</span>
            <?php } elseif($_G['forum_thread']['displayorder'] == -3) { ?> <span>(ไม่สนใจ)</span>
            <?php } elseif($_G['forum_thread']['displayorder'] == -4) { ?> <span>(แบบร่าง)</span>
            <?php } if(!IS_ROBOT && !$_GET['authorid'] && !$_G['forum_thread']['archiveid']) { ?>
<a href="forum.php?mod=viewthread&amp;tid=<?php echo $_G['tid'];?>&amp;page=<?php echo $page;?>&amp;authorid=<?php echo $_G['forum_thread']['authorid'];?>" rel="nofollow" class="blue" style="font-size:12px;font-weight:normal;margin-left:10px;">ดูเฉพาะเจ้าของโพสต์เท่านั้น</a>
<?php } elseif(!$_G['forum_thread']['archiveid']) { ?>
<a href="forum.php?mod=viewthread&amp;tid=<?php echo $_G['tid'];?>&amp;page=<?php echo $page;?>" rel="nofollow" class="blue" style="font-size:12px;font-weight:normal;margin-left:10px;">ดูทั้งหมด</a>
<?php } ?>
...
<img src ="<?php echo IMGDIR;?>/settop.png" title="ปักหมดโพสต์" class="vm" /> จาก <?php echo $post['number'];?><?php echo $postnostick;?>
...
<a href="javascript:;">ผู้เข้าชม <em><?php echo $post['useip'];?><?php if($post['port']) { ?>:<?php echo $post['port'];?><?php } ?></em></a>
<?php } elseif($post['authorid'] && $post['username'] && $post['anonymous']) { if($_G['forum']['ismoderator']) { ?><a href="home.php?mod=space&amp;uid=<?php echo $post['authorid'];?>" target="_blank">ไม่ระบุชื่อ</a><?php } else { ?>ไม่ระบุชื่อ<?php } } else { ?>
<?php echo $post['author'];?> <em>สมาชิกนี้ถูกลบไปแล้ว</em>
<?php } } ?>
</li>
<li class="grey rela">
...
<!-- manage start -->
<?php if($post['first']) { ?>
<em><a href="#moption_<?php echo $post['pid'];?>" class="popup blue">จัดการโพสต์นี้</a></em>
<div id="moption_<?php echo $post['pid'];?>" popup="true" class="manage" style="display:none;">
<?php if(!$_G['forum_thread']['special']) { ?>
<input type="button" value="แก้ไข" class="redirect button" href="forum.php?mod=post&amp;action=edit&amp;fid=<?php echo $_G['fid'];?>&amp;tid=<?php echo $_G['tid'];?>&amp;pid=<?php echo $post['pid'];?><?php if($_G['forum_thread']['sortid']) { if($post['first']) { ?>&amp;sortid=<?php echo $_G['forum_thread']['sortid'];?><?php } } if(!empty($_GET['modthreadkey'])) { ?>&amp;modthreadkey=<?php echo $_GET['modthreadkey'];?><?php } ?>&amp;page=<?php echo $page;?>">
<?php } ?>
<input type="button" value="ลบ" class="dialog button" href="forum.php?mod=topicadmin&amp;action=moderate&amp;fid=<?php echo $_G['fid'];?>&amp;moderate[]=<?php echo $_G['tid'];?>&amp;operation=delete&amp;optgroup=3&amp;from=<?php echo $_G['tid'];?>">
<input type="button" value="ปิด" class="dialog button" href="forum.php?mod=topicadmin&amp;action=moderate&amp;fid=<?php echo $_G['fid'];?>&amp;moderate[]=<?php echo $_G['tid'];?>&amp;from=<?php echo $_G['tid'];?>&amp;optgroup=4">
<input type="button" value="แบนโพสต์" class="dialog button" href="forum.php?mod=topicadmin&amp;action=banpost&amp;fid=<?php echo $_G['fid'];?>&amp;tid=<?php echo $_G['tid'];?>&amp;topiclist[]=<?php echo $_G['forum_firstpid'];?>">
<input type="button" value="เพิ่มการเตือน" class="dialog button" href="forum.php?mod=topicadmin&amp;action=warn&amp;fid=<?php echo $_G['fid'];?>&amp;tid=<?php echo $_G['tid'];?>&amp;topiclist[]=<?php echo $_G['forum_firstpid'];?>">
</div>
<?php } else { ?>
<em><a href="#moption_<?php echo $post['pid'];?>" class="popup blue">จัดการโพสต์นี้</a></em>
<div id="moption_<?php echo $post['pid'];?>" popup="true" class="manage" style="display:none;">
<input type="button" value="แก้ไข" class="redirect button" href="forum.php?mod=post&amp;action=edit&amp;fid=<?php echo $_G['fid'];?>&amp;tid=<?php echo $_G['tid'];?>&amp;pid=<?php echo $post['pid'];?><?php if(!empty($_GET['modthreadkey'])) { ?>&amp;modthreadkey=<?php echo $_GET['modthreadkey'];?><?php } ?>&amp;page=<?php echo $page;?>">
<?php if($_G['group']['allowdelpost']) { ?><input type="button" value="ลบโพสต์" class="dialog button" href="forum.php?mod=topicadmin&amp;action=delpost&amp;fid=<?php echo $_G['fid'];?>&amp;tid=<?php echo $_G['tid'];?>&amp;operation=&amp;optgroup=&amp;page=&amp;topiclist[]=<?php echo $post['pid'];?>"><?php } if($_G['group']['allowbanpost']) { ?><input type="button" value="แบนโพสต์" class="dialog button" href="forum.php?mod=topicadmin&amp;action=banpost&amp;fid=<?php echo $_G['fid'];?>&amp;tid=<?php echo $_G['tid'];?>&amp;operation=&amp;optgroup=&amp;page=&amp;topiclist[]=<?php echo $post['pid'];?>"><?php } if($_G['group']['allowwarnpost']) { ?><input type="button" value="เตือน" class="dialog button" href="forum.php?mod=topicadmin&amp;action=warn&amp;fid=<?php echo $_G['fid'];?>&amp;tid=<?php echo $_G['tid'];?>&amp;operation=&amp;optgroup=&amp;page=&amp;topiclist[]=<?php echo $post['pid'];?>"><?php } ?>
</div>
<?php } ?>
<!-- manage end -->
<?php } if($post['first']) { ?>
<em><a href="home.php?mod=spacecp&amp;ac=favorite&amp;type=thread&amp;id=<?php echo $_G['tid'];?>" class="favbtn blue" <?php if($_G['forum']['ismoderator']) { ?>style="margin-right:10px;"<?php } ?>>บุ๊คมาร์ก</a></em>
<?php } ?>
...
<div class="message">
                	<?php if($post['warned']) { ?>
                        <span class="grey quote">ได้รับการเตือน</span>
                    <?php } ?>
                    ...
                    <?php if($_G['adminid'] != 1 && $_G['setting']['bannedmessages'] & 1 && (($post['authorid'] && !$post['username']) || ($post['groupid'] == 4 || $post['groupid'] == 5) || $post['status'] == -1 || $post['memberstatus'])) { ?>
                        <div class="grey quote">หมายเหตุ: <em>ผู้โพสต์ถูกแบนหรือถูกลบ โพสต์นี้ถูกปิดโดยอัตโนมัติ</em></div>
                    <?php } elseif($_G['adminid'] != 1 && $post['status'] & 1) { ?>
                        <div class="grey quote">หมายเหตุ: <em>กระทู้นี้ถูกแบนห้ามไม่ให้เผยแพร่ต่อสาธารณะ</em></div>
                    <?php } elseif($needhiddenreply) { ?>
                        <div class="grey quote">เนื้อหาถูกซ่อนไว้ สามารถมองเห็นได้เฉพาะเจ้าของโพสต์เท่านั้น</div>
                    <?php } elseif($post['first'] && $_G['forum_threadpay']) { include template('forum/viewthread_pay'); } else { ?>
                    	...
                    	<?php if($_G['setting']['bannedmessages'] & 1 && (($post['authorid'] && !$post['username']) || ($post['groupid'] == 4 || $post['groupid'] == 5))) { ?>
                            <div class="grey quote">หมายเหตุ: <em>ผู้โพสต์ถูกแบนหรือถูกลบ กระทู้นี้ถูกล็อคโดยอัตโนมัติ สามารถมองเห็นเฉพาะผู้ดูแลระบบเท่านั้น</em></div>
                        <?php } elseif($post['status'] & 1) { ?>
                            <div class="grey quote">หมายเหตุ: <em>กระทู้นี้ถูกแบนห้ามไม่ให้เผยแพร่ต่อสาธารณะ สามารถมองเห็นเฉพาะผู้ดูแลระบบเท่านั้น</em></div>
                        <?php } ?>
                        <?php if($_G['forum_thread']['price'] > 0 && $_G['forum_thread']['special'] == 0) { ?>
                            จ่ายเงินค่ากระทู้ ราคา: <strong><?php echo $_G['forum_thread']['price'];?> <?php echo $_G['setting']['extcredits'][$_G['setting']['creditstransextra']['1']]['unit'];?><?php echo $_G['setting']['extcredits'][$_G['setting']['creditstransextra']['1']]['title'];?> </strong> <a href="forum.php?mod=misc&amp;action=viewpayments&amp;tid=<?php echo $_G['tid'];?>" >ดูบันทึกผู้ซื้อ</a>
                        <?php } ?>
...
<?php if(($_G['setting']['mobile']['mobilesimpletype'] == 0) && (!$needhiddenreply)) { if($post['attachment']) { ?>
               ...
               ไฟล์แนบ: <em><?php if($_G['uid']) { ?>กลุ่มสมาชิกของคุณไม่สามารถดาวน์โหลดหรือดูไฟล์แนบ<?php } else { ?>คุณจำเป็นต้อง<a href="member.php?mod=logging&amp;action=login">ลงชื่อเข้าใช้</a>ก่อนจึงจะสามารถดูและดาวน์โหลดไฟล์แนบได้ หากยังไม่มีบัญชีหรือยังไม่ได้เป็นสมาชิก กรุณา<a href="member.php?mod=<?php echo $_G['setting']['regname'];?>" title="ลงทะเบียนใหม่"><?php echo $_G['setting']['reglinkname'];?></a><?php } ?></em>
               </div>
...
<input type="button" class="redirect button" href="forum.php?mod=post&amp;action=reply&amp;fid=<?php echo $_G['fid'];?>&amp;tid=<?php echo $_G['tid'];?>&amp;repquote=<?php echo $post['pid'];?>&amp;extra=<?php echo $_GET['extra'];?>&amp;page=<?php echo $page;?>" value="ตอบกลับ">
</div>
<?php } ?>
       </div>
   </div>
...
<option value="">ดูเครดิต</option>
<option value="0">เป็นกลาง</option>
<option value="1">เห็นด้วย</option>
<option value="2">ไม่เห็นด้วย</option>
</select>
</li>
<?php } ?>
<li><input type="text" value="พิมพ์ข้อความ เพื่อตอบกลับกระทู้นี้" class="input grey" color="gray" name="message" id="fastpostmessage"></li>
...
<?php } ?><input type="button" value="ตอบกลับ" class="button2" name="replysubmit" id="fastpostsubmit"><a href="forum.php?mod=post&amp;action=reply&amp;fid=<?php echo $_G['fid'];?>&amp;tid=<?php echo $_G['tid'];?>&amp;reppost=<?php echo $_G['forum_firstpid'];?>&amp;page=<?php echo $page;?>" class="y" style="height:30px;width:30px;margin-top:7px;background:url(<?php echo STATICURL;?>image/mobile/images/icon_photo.png) no-repeat"><span class="none">ตอบกลับ</span></a><?php if(!empty($_G['setting']['pluginhooks']['viewthread_fastpost_button_mobile'])) echo $_G['setting']['pluginhooks']['viewthread_fastpost_button_mobile'];?></li>
</ul>
</div>
    </form>
</div>
...
<?php if(!$_G['uid']) { ?>
popup.open('คุณยังไม่ได้เข้าสู่ระบบ ต้องการลงชื่อเข้าใช้ตอนนี้หรือไม่', 'confirm', 'member.php?mod=logging&action=login');
<?php } else { ?>
popup.open('คุณไม่มีสิทธิ์โพสต์ตอบกลับ', 'alert');
<?php } ?>
this.blur();
});
<?php } else { ?>
...
if(obj.attr('value') == '') {
obj.addClass('grey');
obj.attr('value', 'พิมพ์ข้อความ เพื่อตอบกลับกระทู้นี้');
obj.attr('color', 'gray');
}
});
<?php } ?>
...
if(msgobj.val() == 'พิมพ์ข้อความ เพื่อตอบกลับกระทู้นี้') {
msgobj.attr('value', '');
}
$.ajax({
type:'POST',
...
if(!message) {
message = 'โพสต์ต้องรอการตรวจสอบ โพสต์ของคุณจะปรากฏขึ้นหลังจากผ่านการตรวจสอบ';
}
...
<a href="javascript:;" title="ขึ้นไปด้านบน" class="scrolltop bottom"></a><?php include template('common/footer'); ?>